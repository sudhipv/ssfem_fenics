...
!
! changes:
! Feb 18, 2016 : SubAssembleMatrix : added to assemble subdomain level block matrices 
! Aug 01, 2016 : asseblematrix     : assemble entire subdomain level matirx
!!---------------------------------------------------------------------------------------
module assembly
...
subroutine SubAssembleMatrix(Adii,Adgg,Adgi,points,edges,triangles,np,ne,nt,nb,ndim,npceout,&
               nomga,eq,meanc,omegas,multipliers,sigma,amps,dbounds,mIndex,sIndex,casei,term)
    ...
    integer :: np, ne, nt, eq, i, j, k, l, term,casei,ndim,npceout,nomga
    ...
    do i = 1,nt
        ...
        !! Assemble matrix over triangles
        do j=1,3
        pt = midpoints(:,j)
        ...
        call advdiff(integral,u,v,du,dv,dx,0.0d0,x,eq,0.0d0,meanc,ndim,npceout,nomga, &
                     omegas,multipliers,sigma,amps,dbounds,casei,term,mIndex,sIndex,0)
...
!            Adig(ii,(jj-ni)) = Adig(ii,(jj-ni)) + intT
    ...
    do i = 1,ne
        ...
        call advdiff(integral,u,v,[0.0d0,0.0d0],[0.0d0,0.0d0],0.0d0,ds,x,eq,0.0d0,meanc,ndim,npceout,nomga, &
                     omegas,multipliers,sigma,amps,dbounds,casei,term,mIndex,sIndex,edge(3))
