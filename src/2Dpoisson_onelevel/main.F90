...
!!MESH-Data-Inputs:
    integer :: np, ne, nt, nb, npg, neg, ntg, nbg, nParts
...
!! Dynamic array allocation
    allocate(p(2,np),e(3,ne),t(3,nt),ijk(ncijk,3),cijk(ncijk))
    allocate(Ui((np-nb)*npceout),Ub(nb*npceout),U_gi(npg),Ub_g(nbg*npceout),U_g(npg))
    allocate(var_g(npg),Af(np,npceout+nmeasg),Afb(nbg,npceout+nmeasg),measnodes(nmeas),d(nmeas))
             !!, & ,uvecb(nbg),uveci(np-nb)                                        !!** PCKF ***
!!-----------------------------------------------------------------------------------------------
    if (pid .eq. 0) then
...
!!-----------------------------------------------------------------------------------------------
!    !! Calling Solver Only for Mallocs Calculations
...
!!-----------------------------------------------------------------------------------------------
    !! New-Solve (combines mallocs & solution)
    call PETSc_stolpcgmShort(pid,np,nb,nbg,npcein,npceout,ncijk,ijk,cijk,Ui,Ub,Ub_g, &
