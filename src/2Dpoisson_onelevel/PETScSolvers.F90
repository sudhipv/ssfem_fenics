...
SUBROUTINE PETSc_stolpcgm(pid, p,e,t,np,ne,nt,nb,nbg,ndim,npcein,npceout,nomga,ncijk, &
...
!! Fortran-Data Declaration
    integer :: i, pid, npceout, ndim, ncijk, maxiter, ierr, nomga, npcein
    integer :: np, ne, nt, nb, nbg, nip, nbp, nbgp, casep, mallocsCals
    ...
    double precision :: rho_next, rho_curr, alpha, beta, err, tol
    double precision :: const_diff, sigma, amp, NegOne
    ...
    double precision, dimension(nb*npceout)      :: Zb
...
!    call GetMallocs(pid, p, e, t,np,ne,nt,nb,ndim,npcein,npceout,nomga,nip,nbp, &
!                    casep,ncijk,ijk,cijk,dbounds,const_diff,omegas,multipliers, &
!                    sigma,mIndex,sIndex,ierr)
    call GetMallocsShort(pid,np,nb,npceout,nip,nbp,ierr)
...
!! These routine has both (in-house Fortran or FEniCS-Dolfin) capabilities inherited
    call StoMatSeqOneLevel(pid,Asii,Asgg,Asgi,p,e,t,np,ne,nt,nb,ndim,npcein,npceout,nomga,nip,nbp, &
                           ...
                           mIndex,sIndex,ierr)
    !call StoMatSeqOneLevelShort(pid,Asii,Asgg,Asgi,np,nb,npcein,npceout,nip,nbp,ncijk,ijk,cijk,ierr)
...
!!!---------------------------------------------------------------------------------------------
    ...
    DO i = 1,maxiter
        ...
        call MatMult(Asgg,PetVecb,PQb,ierr)
        call VecAXPY(PQb,NegOne,SolVecb,ierr)
...
SUBROUTINE PETSc_stolpcgmShort(pid,np,nb,nbg,npcein,npceout,ncijk,ijk,cijk,Ui,Ub,Ub_g, &
...
#include <petsc/finclude/petscpc.h>
    ...
    END SUBROUTINE PETSc_stolpcgmShort
