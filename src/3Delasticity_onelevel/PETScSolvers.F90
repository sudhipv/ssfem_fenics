...
!! Fortran-Data Declaration
    integer :: i, pid, npceout, ncijk, maxiter, ierr, npcein, nvec
    integer :: np, nb, nbg, nip, nbp, nbgp, mallocsCals

    integer, dimension(nb*nvec*npceout)     :: tempb
    integer, dimension((np-nb)*nvec*npceout):: tempi
...
!! PETSc-Initialize
    ...
    nip = (np-nb)*nvec*npceout      !! sto-interior nodes
...
!! Assemble only Subdomain-Level Deterministic matrices to calculate Mallocs
    ...
    IF (mallocsCals .eq. 1) then
        ...
        call GetMallocs(pid,np,nb,npceout,nip,nbp,nvec,ierr)
...
!! These routine has both (in-house Fortran or FEniCS-Dolfin) capabilities inherited
    ...
    !                       mIndex,sIndex,ierr)
    call StoMatSeqOneLevel(pid,Asii,Asgg,Asgi,np,nb,npcein,npceout,nip,nbp,ncijk,ijk,cijk,nvec,ierr)

    !call StoVecSeqOneLevel(pid,Fsi,Fsg,p,e,t,np,ne,nt,nb,ndim,npceout,nomga,nip,nbp,amp,dbounds, &
    !                        mIndex,sIndex,ierr)
    call StoVecSeqOneLevel(pid, Fsi, Fsg, np, nb, npceout, nip, nbp, nvec, ierr)

!if (pid == 0) then
!    call VecView(Fsi,PETSC_VIEWER_STDOUT_SELF, ierr)
