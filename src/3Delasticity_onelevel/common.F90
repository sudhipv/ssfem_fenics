...
!!*********************************************************
!! Subroutine
subroutine writevtk3D(filename,p,t,u,np,nt)

integer :: np, nt, i
double precision, dimension(np) :: u
double precision, dimension(3,np) :: p
integer, dimension(4,nt) :: t
character(len=255) :: filename, str1, str2
...
write(1001,'(a)') 'POLYGONS   ' // trim(str1) // '    ' // trim(str2)
do i = 1,nt
write(1001,*) 4 , t(1,i)-1 ,t(2,i)-1, t(3,i)-1, t(4,i)-1 
enddo
write(1001,'(a)') ' '
call int2str(str1,np,0)
write(1001,'(a)') 'POINT_DATA    ' // trim(str1)
...
subroutine getub(pid,nb,nbg,npceout,Ub_g,Ub)
   ...
   call int2str(str1,pid+1,4)
   !!str1 = '../../data/meshData/bnodes' // trim(str1) // '.dat'    !!** Fortran-data
   str1 = '../../data/meshData/nbnodes' // trim(str1) // '.dat'  !!** Matlab-data
