...
SUBROUTINE StoVecSeqOneLevel(pid, Fsi, Fsg, np, nb, npceout, nip, nbp, nvec, ierr)
...
SUBROUTINE StoMatSeqOneLevel(pid,Asii,Asgg,Asgi,np,nb,npcein,npceout,nip,nbp,ncijk,ijk,cijk,nvec,ierr)
...
#include <petsc/finclude/petscmat.h>
    ...
    integer :: i, j, k, indexi, nid, ncijk, pid
    integer :: np, nb, npceout, npcein, nvec, nbd
...
!!-----------------------------------------------------------------------------------------------
    ...
    allocate(Adii(nid,nid), Adgg(nbd,nbd), Adgi(nbd,nid))
    allocate(nnzi(nip),nnzb(nbp),nnzbi(nbp))

    call int2str(str1,pid+1,4)
    str2 = '../../data/mallocData/nnzi' // trim(str1) // '.dat'
    open(unit=1,file=str2,status='old')
    read(unit=1,fmt=*) nnzi
    close(1)

    str2 = '../../data/mallocData/nnzb' // trim(str1) // '.dat'
    open(unit=2,file=str2,status='old')
    read(unit=2,fmt=*) nnzb
    close(2)

    str2 = '../../data/mallocData/nnzbi' // trim(str1) // '.dat'
    open(unit=3,file=str2,status='old')
...
SUBROUTINE GetMallocs(pid,np,nb,npceout,nip,nbp,nvec,ierr)
...
#include <petsc/finclude/petscmat.h>
    ...
    PetscInt         nip, nbp, one, iNZ, nvec  !!nzi,nzb,
...
!!-----------------------------------------------------------------------------------------------
    ALLOCATE (idxm(nbd),idxn(nbp))
    ALLOCATE (idxmi(nid),idxni(nip))
    ALLOCATE (idxmbi(nbd),idxnbi(nbp))
    allocate (Adii(nid,nid),Adgg(nbd,nbd),Adgi(nbd,nid))
...
!! Here, using first level non-Zero & cijk structure we extrac second level non-Zero structure
    DO k = 1,npceout
        ...
        !print*,'--------$$-------'
        !print*,idxm
        idxn(((k-1)*nbd+1):(k*nbd)) = nZcijk(k)*idxm
        !print*,'-------**--------'
        !print*,idxn
