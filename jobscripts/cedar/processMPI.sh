...
sed -i -e 's/time=0-00:10/time=0-15:00/g' run_ddm.sh
...
sed -i -e 's/-np 32/-np 64/g' run_ddm.sh
## Below d->dims, lc-> mesh density parameter, p->partitions
sed -i -e 's/job-name=DP_nRV_nNodes_nParts/job-name=3DE_3RV_30KNodes_64Parts/g' run_ddm.sh
sed -i -e 's#output=%x-%j.out#output=/scratch/sudhipv/sudhipv/ssfem_fenics/data/slurm/%j-%x.out#g' run_ddm.sh
