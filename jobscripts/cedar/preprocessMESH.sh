...
then
    ...
    cp jobscritps/generate_data_graham2D.sh generate_data_graham.sh
elif [ $input1 == 2 ]
then
    echo "GMSH: 3D-Poisson Selected"
    cp geofiles/slenderBeam.geo foo.geo
    cp jobscritps/generate_data_graham3D.sh generate_data_graham.sh
elif [ $input1 == 3 ]
then
...
sed -i -e 's/time=0-00:30/time=0-01:30/g' generate_data_graham.sh
sed -i -e 's/NP=32/NP=300/g' generate_data_graham.sh
